...
export class RoverfoxClient {
    ...
    private async launchInstance(
        ...
        try {
            await this.connectionPool.safeSend(replayWs, { type: "register-profile", uuid: browserId });
        } catch (error) {
            // Continue execution as this is not critical
        }

        if (!skipAudit) logActionAudit(this.supabaseClient, browserId, "openContext", {});
        ...
        context.close = async () => {
            if (!skipAudit) await logActionAudit(this.supabaseClient, browserId, "closeContext", {});
            ...
            if (tracker) {
                ...
                try {
                    const { error } = await this.supabaseClient.from("data_usage").insert({
            ...
            try {
                await this.connectionPool.safeSend(replayWs, { type: "unregister-profile", uuid: browserId });
            } catch (error) {
     ...
     */
    async createProfile(proxyUrl: string, proxyState: string | null): Promise<RoverFoxProfileData> {
